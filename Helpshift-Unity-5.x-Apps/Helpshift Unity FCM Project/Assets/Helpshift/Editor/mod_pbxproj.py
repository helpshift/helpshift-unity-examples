...
class XCBuildConfiguration(PBXType):
    def add_search_paths(self, paths, base, key, recursive=True, escape=True):
        ...
        for path in paths:
            ...
            #added by helpshift - Needed to remove old SDK library search path
            for value in self[base][key]:
                if "helpshift" in value:
                    self[base][key].remove(value)
            #end helpshift
...
class XcodeProject(PBXDict):
    ...
    def remove_file(self, file, recursive=True):
        if not PBXType.IsGuid(file):
            id = file.id
        else:
            id = file
        ...
        if id in self.objects:
            ...
            if 'children' in file:
                for child in file['children']:
                    self.remove_file(self.get_obj(child))
            self.modified = True
